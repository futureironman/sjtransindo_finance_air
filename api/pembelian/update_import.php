...
/// DELETE ALL
        $a = pg_fetch_array(pg_query($conn, "SELECT total, created_at,suplier, uid_inv_header FROM inv_detail_pembelian  WHERE id='$_POST[id]'"));
        $uid_akun_terima = '0709a09f-6317-6fbb-6541-670a30dc5657';
        
        // update saldo terkini Customer
        pg_query($conn, "UPDATE keu_akun SET saldo_terkini= (saldo_terkini - $a[total]) WHERE uid='$a[uid_suplier]'");
        ...
        $sql="UPDATE keu_akun_log SET deleted_at='$waktu_sekarang' WHERE  created_at = '$a[created_at]' and (uid_akun_efek='$a[uid_suplier]' OR uid_akun_efek='$uid_akun_terima')"; 
        pg_query($conn, $sql);
        ...
        $sql= "UPDATE keu_akun_log SET saldo= (saldo - $a[total]) WHERE uid_akun='$a[uid_suplier]' AND created_at > '$a[created_at]'";
        pg_query($conn, $sql);
      ...
      $a = pg_fetch_array(pg_query($conn, "SELECT a.total, a.uid_inv_header, b.uid_data as uid_supplier, a.tanggal_pembelian FROM inv_detail_pembelian a, keu_akun b WHERE a.uid_inv_header='$uid_inv_header' and a.uid_suplier=b.uid_data  and b.linked_table = 'supplier' AND NOT EXISTS (SELECT NULL FROM keu_akun_log c where c.uid_akun=b.uid and c.id_data = '$uid_inv_header')"));
        ...
        $sql="INSERT INTO keu_akun_jurnal (waktu, uid_data, keterangan, id_jenis, no_bukti) VALUES ('$waktu', '$uid_inv_header', 'Penjualan', '4', '$uid_inv_header')";
        pg_query($conn,$sql);	    
        ...
        $sql = "INSERT INTO keu_akun_log (uid_akun, created_at, id_data, id_status, keterangan, debet, kredit, saldo,tabel,uid_akun_efek) VALUES ('$uid_supplier', '$waktu', '$uid_inv_header', '25', '$uid_inv_header','$total', '0', '$saldo_kredit', 'invoice_header','$uid_akun_terima')";
