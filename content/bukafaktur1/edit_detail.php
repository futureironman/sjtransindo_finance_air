...
$a=pg_fetch_array(pg_query($conn,"SELECT * FROM keu_buka_faktur_detail WHERE uid='$_POST[uid]'"));
?>
<form action="aksi-edit-detail-bukafaktur" method="POST" enctype="multipart/form-data">
	<div class="modal-dialog modal-md a-lightSpeed">
		<div class="modal-content">
         ...
         <div class="form-group focused">
            <input type="hidden" value="<?= $a["uid"] ?>" name="uid">
            <input type="hidden" value="<?= $a["uid_akun_bank"] ?>" name="uid_akun_bank">
              
            <div class="form-group focused">
                    <label class="form-control-label">Jenis Akun</label>
                    <select name="uid_akun" class="form-control modal_select2">
						<?php
						$tampil=pg_query($conn,"SELECT * FROM keu_akun");
