<?php
$data = pg_fetch_array(pg_query($conn, "SELECT MAX(no_bukti) as noBukti FROM keu_pembayaran"));
        ...
        <div class="card card-form">
            <div class="row no-gutters">
                <div class="col-lg-4 card-body">
                    ...
                    <input type="hidden" name ="no_bukti" id="no_bukti" value="<?php echo 'PP.000'.$noBukti?>">
                    
                    <h3 style="color: red !important; font-weight: bold;"><?php echo 'PP.000'.$noBukti?></h3>
                </div>
                    ...
                    <div class="row form-group">
                        ...
                        <div class="col-md-8">
                            <select name="uid_customer" id="id_customer" class="form-control select2" required>
                                ...
                                <?php
                                $tampil = pg_query($conn, "SELECT a.uid as uid, a.nama FROM customer a, keu_akun b WHERE cast(a.uid as UUID)=b.uid_data ORDER BY a.nama");
            ...
            <div class="row no-gutters">
                <div class="col-lg-4 card-body">
                    ...
                    <div class="col-lg-12 card-body">
                        ...
                        <input type="hidden" name ="deposit_value" id="deposit_value" value="<?php echo $noBukti?>">
                        <h3 style="color: red !important; font-weight: bold;"><?php echo 'Rp.'.$noBukti?></h3>
                    </div>
                </div>
                <div class="col-lg-8 card-form__body card-body">
                    <div class="row form-group">
                ...
                for (var a = 0; a < response.length; a++) {
                    if (response) {
                        $(".data_invoice tbody").append("<tr>" +
                            "<td> <input type=\"checkbox\" class=\"get_value\" name='customer[]' onclick =\"check(" + a + "," + (response[a]["total"] - response[a]["jumlah_terbayar"]) + ")\" value=" + no + " id=\"erman\"/></div></td>" +
                            "<td>" + response[a]["jatuh_tempo"] + "</td>" +
                            "<td>" + response[a]["invoice_number"] + "<input type=\"hidden\" class=\"form-control\" name='invoice_number_"+ no +"' value=\""+ response[a]["invoice_number"] +"\"/></td>" +
                            "<td>" + response[a]["po_house_number"] + " <input type=\"hidden\" class=\"form-control\" name=\"uid_"+ no +"\" value=\""+response[a]["uid_invoice_header"]+"\"/></td>" +
                            "<td>" + response[a]["nama_pegawai"] + "</td>" +
                            "<td>" + number_format(response[a]["total"]) + "<input type=\"hidden\" class=\"form-control\" name=\"sisa_bayar_"+ no +"\" value=\""+ response[a]["sisa_bayar"] +" \"/></td>" +
                            "<td>" + number_format(response[a]["jumlah_terbayar"]) + "</td>" +
                            "<td><input type=\"text\" class=\"form-control uang text-right number\" id=\"number" + a + "\" name=\"number_"+ no +"\")\" onclick =\"nilai_pembayaran(" + a + "," + (response[a]["total"] - response[a]["jumlah_terbayar"]) + ")\" disabled/></td>" );
                    }
                    no++;
