...
<?php
        ...
        // var_dump($_POST['baru']);
            ...
            foreach ($_POST['customer'] as $check) {
                ...
                $sql = "UPDATE invoice_header SET status_pembayaran= 'true' , jenis_pembayaran = 'true', jumlah_terbayar = (jumlah_terbayar + $jumlah), sisa_bayar=(sisa_bayar - $sisa),  is_lunas='$is_lunas' WHERE uid='$uid'";
                pg_query($conn, $sql);
                ...
                //   -------------- END HISTORY KAS KECIL ---------------------------
                    ...
                    // insert table keu_deposit
                    if($uid= '404b8011-432b-d985-15aa-4d1cf7cb5878'){
                        //INSERT TABLE DEPOSIT
                        $a = pg_fetch_array(pg_query($conn, "SELECT * FROM keu_deposit where uid_data='$_POST[uid_customer]'"));
                        if($a["uid"] == ''){
                            $sql="INSERT INTO keu_deposit (uid_akun, uid_data, saldo, created_at) VALUES ('$uid_data', '$_POST[uid_customer]', '$jumlah', '$waktu')";
                            pg_query($conn,$sql);
                        }
                        else{
                            $sql = "UPDATE keu_deposit set saldo =(saldo + $jumlah), created_at='$waktu' where uid_data='$_POST[uid_customer]'";  
                            pg_query($conn, $sql);
                        }
                    }
            }
	...
	elseif($act=='delete'){
        ...
        while($r=pg_fetch_array($cek)){
            ...
            $sql = "UPDATE keu_deposit set saldo =(saldo - $jumlah) where created_at='$waktu' and udi_data='$uid_customer'";  
            pg_query($conn, $sql);
        }
