...
?>
                ...
                <div class="flex">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Terima Pembayaran</li>
                        </ol>
                    </nav>
                    <h4 class="m-0">Terima Pembayaran</h4>
                </div>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-hover" style="width:100%" id="example">
                            ...
                            $grand_total = 0;
                            $grand_total_bayar = 0;
                            $tampil = pg_query($conn, "SELECT * from keu_pembayaran where deleted_at is NULL and tanggal BETWEEN '$tanggal_awal 00:00:00' AND '$tanggal_akhir 23:59:59'");
                            while ($r = pg_fetch_array($tampil)) {
                                $a = explode(" ", $r['tanggal']);
                                $waktu = DateToIndo2($a[0]) . ' ' . $a[1];
                                $b = pg_fetch_array(pg_query($conn, "SELECT uid_data FROM keu_akun WHERE uid='$r[uid_customer]'"));
                                $customer = pg_fetch_array(pg_query($conn, "SELECT nama FROM customer WHERE uid='$b[uid_data]'"));
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <button type="button" rel="tooltip" class="btn btn-sm btn-danger btnHapus-terimabayar" data-original-title="Hapus" title="Hapus" id="<?= $r["uid"] ?>"><i class="fa fa-trash"></i></button>
                                    </td>
        ...
        <script>
            ...
            $("body").on("click", ".btnHapus-terimabayar", function() {
                var id = this.id;
                swal({
                        title: "Are you sure?",
                    ...
                    function(isConfirm) {
                        if (isConfirm) {
                            swal({
                                ...
                                function() {
                                    document.location.href = 'aksi-hapus-terimabayar-' + id;
                                });
                        } else {
