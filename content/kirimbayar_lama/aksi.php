...
<?php
        ...
        //Aksi pembayaran di table invoice_header   
            ...
            foreach ($_POST['supplier'] as $check) {
                ...
                if($deposit > 0){
                    ...
                    $pembayaran_sebelum = pg_fetch_array(pg_query($conn, "SELECT id, saldo FROM keu_akun_log WHERE uid_akun='404b8011-432b-d985-15aa-4d1cf7cb5878' AND created_at < '$waktu' ORDER BY id DESC LIMIT 1"));
                    $pembayaran_sebelum = $pembayaran_sebelum["saldo"];
                    ...
                    $pembayaran_sebelum = pg_fetch_array(pg_query($conn, "SELECT id, saldo FROM keu_akun_log WHERE uid_akun='$uid_akun_bayar' AND created_at < '$waktu' ORDER BY id DESC LIMIT 1"));
                    $pembayaran_sebelum = $pembayaran_sebelum["saldo"];
                    $saldo_debet = $pembayaran_sebelum + $deposit;
    
                    //PENCATATAN DI LOG KEU AKUN KAS KECIL (BCA)
                    $sql = "INSERT INTO keu_akun_log (uid_akun, created_at, id_status, keterangan, debet, kredit, saldo, tabel,uid_akun_efek) VALUES ('$uid_akun_bayar', '$waktu', '25', '$no_bukti', '$deposit', '0', '$saldo_debet', 'keu_depoist', '$uid_akun_bayar')";
                    pg_query($conn, $sql);
    
                    // Update keuangan KAS(BCA) sesudah tanggal
                    pg_query($conn, "UPDATE keu_akun SET saldo_terkini= (saldo_terkini + $deposit) WHERE uid='$uid_akun_bayar'");
                    pg_query($conn, "UPDATE keu_akun_log SET saldo= (saldo + $deposit) WHERE uid_akun='$uid_akun_bayar' AND created_at > '$waktu' ");
                 //   -------------- END HISTORY KAS KECIL ---------------------------
                ...
                foreach ($_POST['supplier'] as $check) {
                    $id = $_POST["id_$check"];
                    // Update invoice_header                
                    $sql = "UPDATE inv_detail_pembelian SET  is_lunas='Y' WHERE id='$id' where uid_data='$uid'";
                    pg_query($conn, $sql);
                }
            }  
	...
	elseif($act=='delete'){
        ...
        if($deposit > 0){
            ...
            pg_query($conn, "UPDATE keu_akun_log SET saldo= (saldo + $deposit) WHERE uid_akun='404b8011-432b-d985-15aa-4d1cf7cb5878' AND created_at > '$waktu' ");
            // --------------------------------
            pg_query($conn, "UPDATE keu_akun SET saldo_terkini= (saldo_terkini - '$deposit') WHERE uid='$a[uid_akun_bayar]'");
            pg_query($conn, "UPDATE keu_akun_log SET saldo= (saldo - $deposit) WHERE uid_akun='$a[uid_akun_bayar]' AND created_at > '$waktu' ");
            // --------------------------------
