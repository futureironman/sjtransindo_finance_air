...
<?php
        ...
        $d = pg_fetch_array(pg_query($conn, "SELECT MAX(no_bukti) as nomor FROM keu_pembelian where id_divisi='$_SESSION[divisi]' and deleted_at is NULL "));
        $kode_before = substr($d['nomor'],0,7);
        $kode_now="PHL.$thn.";
        if($kode_before==$kode_now){
            $no_urut = (int) substr($d['nomor'],6,7);
            $no_urut++;
        ...
        if (!empty($_POST['supplier'])) {
            ...
            pg_query($conn, "INSERT INTO keu_pembelian (uid_supplier, id_metode_bayar, uid_akun_bayar, total_tagihan, total_pembayaran, status_pembayaran, created_at, tanggal, catatan, keterangan_detail,id_invoice_header,no_bukti, jumlah_deposit, id_divisi) VALUES ('$uid_supplier', '$id_metode_bayar', '$uid_akun_bayar', '$total_tagihan', '$total_pembayaran', '$status_pembayaran' ,'$waktu_sekarang', '$waktu','$_POST[catatan]','$keterangan_detail','$id_invoice_header','$no_bukti' , '$deposit','$_SESSION[divisi]')");
            ...
            $sql = "INSERT INTO keu_akun_jurnal (waktu, uid_data, keterangan, id_jenis, no_bukti,id_divisi) VALUES ('$waktu', '$uid_pembayaran', 'Pembayaran Pembelian', '1', '$keterangan_detail','2')";
            pg_query($conn, $sql);
            ...
            // ------------------HISTORY LOG supplier--------------
                ...
                $sql = "INSERT INTO keu_akun_log (uid_akun, created_at, id_status, keterangan, debet, kredit, saldo, keterangan_detail, tabel,uid_akun_efek) VALUES ('$uid_akun_bayar', '$waktu', '25', '$no_bukti', '0', '$total_pembayaran', '$saldo_kredit', '$keterangan_detail', 'keu_pembelian', '$uid_supplier')";
                pg_query($conn, $sql);
        ...
        if (!empty($_POST['check_list'])) {
            ...
            foreach ($_POST['check_list'] as $check) {
                ...
                //PENCATATAN DI LOG KEU AKUN SUPPLIER Efek selisih bayar
                
                $sql = "INSERT INTO keu_akun_log (uid_akun, created_at, id_status, debet, kredit, saldo, keterangan, keterangan_detail, tabel,uid_akun_efek) VALUES ('$uid_supplier', '$waktu', '25',  '0', '$jumlah', '$saldo_kredit','$no_bukti', '$keterangan', 'keu_pembayaran','$uid_akun')";
                pg_query($conn, $sql);
            ...
            pg_query($conn, "INSERT INTO pegawai_activity_log(activity, effects_to, created_at, uid_pegawai, tabel) VALUES('insert kirim bayar','$d[uid]','$waktu','$_SESSION[login_user]','keu_pembelian')");
        ...
        // insert pegawai_activity_log
        pg_query($conn, "INSERT INTO pegawai_activity_log(activity, effects_to, created_at, uid_pegawai, tabel) VALUES('delete kirim bayar','$_GET[uid]','$waktu','$_SESSION[login_user]','keu_pembelian')");
